...
|            |                 | ex =>Dog a = new Dog(10); Dog b = new Dog(30); 이 둘이 매개변수로 가능. 단 한번 정해지면 변경 불가.
							  |final class = 상속 불가 / final method = 오버라이딩 불
...
|
										인터페이스의 다중구현 | 인터페이스는 구현부가 없으므로, 다중 상속 시 중복된 메서드도 자식의 오버라이딩 메서드 하나로 통합되어 호출된다.																	| extends와 implements 동시에 사용 가능 => 그렇다면 왜 상속 타입을 굳이 나눠서 받을까?
														...
														| -> 둘 다 동시에 상속이 가능.
														------------------------------------------------------------------
|   2026-02-25	|	좋은 객체지향이란?	|	
										객체지향 프로그래밍 = 여러개의 독립된 단위 (객체) 의 모임으로 파악하여 
										각각의 객체는 서로 정보와 기능을 주고받으며 데이터를 처리할 수 있다.
					객체지향의 장점		| 역할과 구현을 분리한다.
										|
										| 조건 => 클라이언트 (호출자) / 역할 (인터페이스) / 구현대상( 구현한 자식 클래스 )
										 1. 클라이언트는 대상의 역할(인터페이스 / 부모 클래스)만 알면된다.
										 2. 클라이언트는 구현 대상(자식 클래스)의 내부 구조를 몰라도 된다.
										 3. 클라이언트는 구현 대상(자식 클래스)의 내부 구조가 변경되어도 영향을 받지 않는다.
										 4. 클라이언트는 구현 대상(자식 클래스) 자체를 변경해도 영향을 받지 않는다

								OCP원칙| 
								Open for extension : 새로운 기능의 추가나 변경사항이 생겼을 때, 기존 코드는 확장 할 수 있어야 한다.
								Closed for modification : 기존의 코드는 수정되지 않아야 한다.
								=> 확장에는 열려있고, 변경에는 닫혀있다는 뜻.(기존의 코드 수정 없이 새로운 기능을 추가 할 수 있어야 한다.)

							전략 패턴 | 전략패턴을 사용하면 클라이언트 코드의 변경 없이 쉽게 교체할 수 있다.
							전략 패턴이란? 
							메서드를 직접 수정하지 않고, 인터페이스를 통해 외부에서 주입(Injection)받아 동적으로 자유롭게 교체하여 사용하는 패턴.
|
